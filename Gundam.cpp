...
GLdouble vNear = -20;
GLdouble vFar = 20;
GLdouble vDepth = vFar - vNear;
GLdouble vHeight = vTop - vBottom;

const GLdouble vFovDegs = 45.0;
GLdouble vFovRads = vFovDegs * 3.14159 / 180;
...
void ArmCombine() {
	...
	if (isTextureOn) {
		...
		robotUpperArm();
		robotBottomArm();
		texture[1] = loadTexture("MetalBlack.bmp");
		robotHand();
		robotArmJoint();
	}
	else {
		...
		robotUpperArm();
		robotBottomArm();
		texture[1] = loadTexture("black.bmp");
		robotHand();
		robotArmJoint();
	}
...
void robotDrawHead() {
	...
	texture[3] = loadTexture("blueMetal.bmp");
	robotEyes();
	glPopMatrix();
	glDeleteTextures(1, &texture[3]);
...
void robotDrawLeftLeg() {
	...
	if (isTextureOn) {
		...
		robotCalf();
		robotLeg2();
		texture[1] = loadTexture("blueMetal.bmp");
		robotKnee();
		texture[2] = loadTexture("redMetal.bmp");
		robotLeg();
		glPopMatrix();
		...
		robotCalf();
		robotLeg2();
		texture[1] = loadTexture("blue.bmp");
		robotKnee();
		texture[2] = loadTexture("red.bmp");
		robotLeg();
		glPopMatrix();
...
void projection()
{
	...
	if (isOrtho == true) {
		...
		vNear = (vHeight / 2.0) / tan(vFovRads / 2.0);
		vFar = vNear + vDepth;

		glFrustum(vLeft, vRight, vBottom, vTop, vNear, vFar);
		glTranslated(0, 0, -72.4264072861352);
